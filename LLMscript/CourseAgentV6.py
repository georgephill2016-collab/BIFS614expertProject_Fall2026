...
while True:
    ...
    TOP_K = 5
    start = time.time()
    context = retrieve_context(user_prompt, TOP_K)
    print("Context(user prompt + retrieved course content) chars:", len(context))
    print("Retrieval:", time.time() - start, "seconds")
    final_prompt = f"""
    <|system|>
    ...
    2. If the answer is clearly found in the context, answer it.
    ...
    with torch.no_grad():
        output_ids = model.generate(
            input_ids, max_new_tokens=350, do_sample=True, temperature=0.2, top_p=0.9, repetition_penalty=1.1,
            pad_token_id=tokenizer.eos_token_id
        )
    print("Generation:", time.time() - start, "seconds")
